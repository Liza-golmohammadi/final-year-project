...
# ==============================
# 2. WINDOW-LEVEL METRICS HELPER
# Each window is an independent test sample — no aggregation
# ==============================
def window_level_metrics(y_true, y_pred, y_prob):
...
# ==============================
# 3. HT VARIABILITY BASELINE — WINDOW-LEVEL
# Non-ML: threshold on hold_std learned from training fold only
# ==============================
def run_ht_baseline_window(X, y, groups, title):
    print(f"\n{'='*60}")
    print(title)
    print(f"{'='*60}")
    ...
    for fold, (train_idx, test_idx) in enumerate(gkf.split(X, y, groups)):
        ...
        y_pred = (ht_test >= threshold).astype(int)

        acc, bacc, f1, auc = window_level_metrics(y_test, y_pred, ht_test)
        if acc is None:
        ...
        print(f"\nFold {fold+1}  |  Threshold: {threshold:.4f}  |  Test windows: {len(test_idx)}  |  Test subjects: {test_groups.nunique()}")
...
def run_ht_baseline_cross_dataset_window(X_train_df, y_train, X_test_df, y_test, groups_test, title):
    print(f"\n{'='*60}")
    print(title)
    print(f"{'='*60}")
    ...
    acc, bacc, f1, auc = window_level_metrics(y_test_arr, y_pred, ht_test)

    print(f"Threshold (from Tappy train): {threshold:.4f}")
    print(f"Test windows: {len(y_test_arr)}  |  Test subjects: {groups_test.nunique()}")
    print(f"Window-level  Accuracy: {acc:.4f}  Balanced Acc: {bacc:.4f}  F1: {f1:.4f}  AUC: {auc:.4f}")


# ==============================
# 4. LOGISTIC REGRESSION — WINDOW-LEVEL
# ==============================
lr_model = Pipeline([
...
def run_group_cv_window(model, X, y, groups, title):
    ...
    for fold, (train_idx, test_idx) in enumerate(gkf.split(X, y, groups)):
        ...
        acc, bacc, f1, auc = window_level_metrics(y_test.values, y_pred, y_prob)
        if acc is None:
            print(f"\nFold {fold+1}  |  Skipped (single class in test)")
            continue
        ...
        print(f"\nFold {fold+1}  |  Test windows: {len(test_idx)}  |  Test subjects: {test_groups.nunique()}")
        print(f"Window-level  Accuracy: {acc:.4f}  Balanced Acc: {bacc:.4f}  F1: {f1:.4f}  AUC: {auc:.4f}")
...
run_ht_baseline_window(X_mit, y_mit, g_mit, "Exp 1 MIT-ONLY — HT Baseline (Window-level)")
run_group_cv_window(lr_model, X_mit, y_mit, g_mit, "Exp 1 MIT-ONLY — LR (Window-level)")
...
run_ht_baseline_window(X_tappy, y_tappy, g_tappy, "Exp 2 TAPPY-ONLY — HT Baseline (Window-level)")
run_group_cv_window(lr_model, X_tappy, y_tappy, g_tappy, "Exp 2 TAPPY-ONLY — LR (Window-level)")
...
run_ht_baseline_cross_dataset_window(
...
y_pred_lr = lr_model.predict(X_test_cd)
y_prob_lr = lr_model.predict_proba(X_test_cd)[:, 1]
acc, bacc, f1, auc = window_level_metrics(y_test_cd.values, y_pred_lr, y_prob_lr)
print("\nExp 3 CROSS-DATASET — LR (Window-level)")
print(f"Test windows: {len(y_test_cd)}  |  Test subjects: {g_test_cd.nunique()}")
print(f"Window-level  Accuracy: {acc:.4f}  Balanced Acc: {bacc:.4f}  F1: {f1:.4f}  AUC: {auc:.4f}")
...
run_ht_baseline_window(X_bal, y_bal, g_bal, "Exp 4 BALANCED POOLED — HT Baseline (Window-level)")
run_group_cv_window(lr_model, X_bal, y_bal, g_bal, "Exp 4 BALANCED POOLED — LR (Window-level)")
